...
export type Num<T extends "I" | "U", Size extends NumSize<T>> = Size extends
  16 | 32 ? number : bigint;

export type NumSize<T extends "U" | "I"> = T extends "U" ? 16 | 32 | 64 | 128
...
type Buf<T extends "u8" | "i8" | "f32" | "f64"> = T extends "u8" ? Uint8Array
...
export function make_call<
  T extends "unary" | "sse",
  ReturnTy extends ReturnType<ReturnType<RpcTransport[T]>["call"]>,
  Result,
>(
  rpc: RpcTransport,
  type: T,
  ...
  encoder(d);
  d.flush();
  return decoder(fn.call(requestInit) as any);
}
...
export class Decoder {
  ...
  fixed_buf<T extends "u8" | "i8" | "f32" | "f64">(type: T, len: number) {
    return () => {
      let byteLength = len *
        (type == "u8" || type == "i8" ? 1 : type == "f32" ? 4 : 8);
      return this.#unsafe_read(byteLength, () => {
        switch (type) {
          case "u8":
            return new Uint8Array(this.#view.buffer, this.offset, len) as Buf<
              T
            >;
          case "i8":
            return new Int8Array(this.#view.buffer, this.offset, len) as Buf<T>;
          default:
            let buf = this.#view.buffer.slice(
              this.offset,
              this.offset + byteLength,
            );
            return (type == "f32"
              ? new Float32Array(buf)
              : new Float64Array(buf)) as Buf<T>;
        }
      });
    };
  }
...
export class BufWriter implements Write {
  ...
  f32(num: number) {
    this.#unsafe_write(
      4,
      () => this.#view.setFloat32(this.#written, num, true),
    );
  }
  f64(num: number) {
    this.#unsafe_write(
      8,
      () => this.#view.setFloat64(this.#written, num, true),
    );
  }
  ...
  len_u30(num: number) {
    ...
    let b2 = (num >> 16) & 0xff;
    if (num < (1 << 22)) {
      return this.write(Uint8Array.from([0x80 | b2, b3, b4]));
    }
    let b1 = (num >> 24) & 0xff;
    if (num < (1 << 30)) {
      return this.write(Uint8Array.from([0xC0 | b1, b2, b3, b4]));
    }
    throw new Error("out of range integral type conversion attempted");
  }
}
